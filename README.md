...
| [producer-mindset](producer-mindset/) | 生产者视角商业分析，把产品/服务/生意从消费端翻转到生产端，拆解成本、定价、利润、壁垒 | 分析一下、怎么赚钱、为什么这么贵、生意拆解、成本分析、能不能做、利润在哪 |
| [write-prd](write-prd/) | 需求工程技能，将用户想法转化为结构化 PRD 文档。智能推断 + 核心问题确认，最小化交互次数 | PRD、产品需求文档、需求分析、需求规格、写需求 |
| [frontend-code-review](frontend-code-review/) | 前端代码审查技能，支持待提交变更审查和指定文件审查两种模式。按代码质量、性能、业务逻辑三维度检查，输出带紧急度标记的审查报告 | 代码审查、code review、前端审查、检查代码、review代码 |
...
│   ├── SKILL.md                 # 前端代码审查技能
│   └── references/
│       ├── code-quality.md          # 代码质量规则目录
│       ├── performance.md           # 性能规则目录
│       └── business-logic.md        # 业务逻辑规则目录
├── component-ai-docs/
...
│   ├── SKILL.md                 # 生产者视角商业分析
│   └── references/
│       └── business-model-patterns.md  # 商业模式速查手册
├── write-prd/
...
生产者视角商业分析技能，帮助训练"从消费端翻转到生产端"的思维习惯。核心价值：

- **四刃拆解刀** — 成本结构 / 定价逻辑 / 利润模型 / 竞争壁垒，每次分析的标准框架
- **四条路由** — (A) 单品拆解：一个产品为什么卖这个价；(B) 生意拆解：一家店/品牌怎么赚钱；(C) 行业扫描：一条产业链的利润分布；(D) 机会判断：我能做吗、差距在哪、最小切口是什么
- **教思维不喂结论** — 每次分析必给可复用的思维框架，目标是让用户慢慢不需要这个 skill
- **证据先行** — 所有具体分析配合 WebSearch 获取真实数据，清晰标注搜索来源 vs 逻辑推演
- **商业模式速查** — 内置 9 种盈利模型、各行业成本结构基准、6 种定价模式判定卡、8 种护城河框架

### frontend-code-review

前端代码审查技能，帮助团队在提交前发现代码问题。核心价值：

- **双模式支持** — (A) 待提交变更审查：扫描暂存区/工作区文件，标记违规项；(B) 指定文件审查：对用户指定的文件执行检查清单
- **三维度检查** — 代码质量（条件类名、Tailwind 优先、类名排序）、性能（React Flow 数据使用、复杂 prop 稳定性）、业务逻辑（如 workflowStore 在 Node 组件中的禁用）
- **紧急度分级** — 每个问题标注 `IsUrgent`，审查报告按紧急问题 → 改进建议排序
- **可操作修复建议** — 每个问题附带具体的代码修复方案，审查后可一键询问是否应用修复
