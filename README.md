...
| 技能 | 说明 | 触发词示例 |
|------|------|-----------|
| [project-onboarding](project-onboarding/) | 历史项目 AI 辅助初始化——自动扫描技术栈、目录、请求方式、页面骨架，检测多模式冲突等人决策，生成 ~50 行 AGENTS.md | 项目初始化、AI辅助配置、项目onboarding、项目速览、让AI认识项目 |
...
| [prd-fill](prd-fill/) | 弱模型场景 PRD 填充 + Task 展开。零散需求 → 场景匹配 → 对话梳理 → 模板填充 → Task 列表，衔接 sdesign-gen-* 代码生成技能 | 梳理需求、整理PRD、生成Task、拆分任务、PRD填充 |
...
- **自动扫描** — 读 package.json、tsconfig.json、src/ 目录、样本页面 import，提取技术栈、目录约定、请求方式、组件库、路径别名
- **多模式检测** — 发现多种接口请求方式、状态管理、字典获取并存时，列出每种的出现位置和文件数，标注 🔴 等人决策
- **不替人决策** — 不默认选占比最多的模式，所有冲突一次列出，用户拍板后才生成
- **只生成一个文件** — 不改项目代码，不引入依赖。产出 ~30 行项目速览，够 AI 后续找到代码在哪、照着已有风格模仿
...
弱模型场景下的 PRD 填充 + Task 展开技能。与 `write-prd`（通用 PRD 创建）互补，专注于场景化、结构化的 PRD 生成，直接衔接代码生成 Skill。核心价值：

- **零散需求入口** — 接受信息倾倒（md 片段、接口文档、对话记录），不要求输入格式
- **对话梳理替代蓝图** — AI 输出结构化功能点清单，逐项确认，不遗漏不猜测
- **场景匹配 + 模板填充** — 按场景（CRUD/大屏/工作流/兜底）加载对应 PRD 模板，逐字段填入
- **Task 机械展开** — 每个 PRD 按场景预置 Task 序列展开具体 Task，绑定输出锁 + 闸门 + 代码模板
- **调试坐标系** — PRD 字段 ↔ 模板占位符 ↔ 代码行号 双向可追溯，代码报错时修 PRD 而非修代码
- **兜底安全网** — 无匹配场景时使用最小化模板 + 全局公约，不硬生成无模板保护的代码
