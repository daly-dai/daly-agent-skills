...
- [ ] 是否区分了品类（快消/耐用/服务/平台/制造/农业），按不同逻辑分析？
...
- [ ] 本地知识在结论层面推翻了搜索数据时，是否以本地知识为基准重算而非折中？
- [ ] 是否区分了新店爬坡期和成熟期的收入推演？
- [ ] 盈亏平衡是否给出了三条线（经营打平/投资回本/机会成本）？
- [ ] 是否做了逆向信号排查（好位置没竞品/幸存者偏差/加盟商自卖自夸）？
- [ ] 加盟模式是否拆了"二道税"（显性加盟费 + 隐性供应链加价）？
- [ ] 客群画像是否关联了毛利率推演（什么客群锁死什么毛利水平）？
- [ ] 陌生行业是否执行了"陌生行业分析协议"而非靠猜？
